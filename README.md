...
1. Explain how you got the HTML, with the correct data, displayed in the browser?
   > I am basically going to explain the majority of the steps I took so apologies if this is a long answer. // First step was to work in `PotteryWheel.js` and create the pottery objects calling the function `makePottery` to return each object with `shape`, `weight`, `height`, and `id` properties. This assigns different parameters to each object. // Second, the function `firePottery` is created in `Kiln.js`. This function assigns a true/false boolean property of cracked to objects, determined by the temperature of the kiln. The function also produces a true/false boolean property for if the object was fired. // The next module to establish the correct data is `PotteryCatalog.js` with the function `toSellOrNotToSell`. The function checks if the pottery is cracked and if it isn't, assigns it a `price` property determined by the `weight` property. These objects are then pushed into the `soldPottery` array. // In `PotteryList.js`, the array is converted into HTML with the function `potteryList`. This creates an HTML string. The `potteryList` function is then called in `main.js` with `document.querySelector(".potteryList").innerHTML = potteryList()` targeting the article class element in `index.html`. This renders it to the DOM and returns its inner HTML string with `potteryList()`. This will display the correct data in the HTML and show in the browser.
2. In the **PotteryList** module, when you iterate your pottery, you need to show the evidence of what the **weight** property's value is for the 2nd piece of pottery. Use [Loom](https://www.loom.com/) to record your browser window with the developer tools open and show those values.
   > [Paste your video's public URL here](https://www.loom.com/share/cca3beeee80b438790a004c60246b59f)
3. The **PotteryWheel** module has a single function named `makePottery`. Why doesn't that module have all of the other code in it?
   > This module handles making the pottery. The makePottery function is holding all of the objects and properties that are being created. This module holds that task so if there is a mistake in the code relating to making the pottery(the objects with properties and parameters established in the code foundationally, the code will lack crucial information without this module), it is easy to tell where the bug may be located. 
4. The pottery shop has learned that there is a set of customers that are willing to buy cracked pottery at a discounted price of $2.50. That means that the cracked pottery should now be displayed in the catalog. Explain the changes that this new business strategy would cause to your algorithm.
   > The `PotteryCatalog.js` module will be changed. The logic for the `toSellOrNotToSell` function will be altered so the cracked pottery is added to the array with a `price` property. The if/else goes from `if (firedPottery.cracked === false)` to `if (firedPottery.cracked === true) { firedPottery.price = 2.50; }`, while also including ` } else if (firedPottery.weight >= 6) { firedPottery.price = 40; } else { firedPottery.price = 20; } soldPottery.push(firedPottery);` which handles cracked and uncracked. This if/else structure determines first if the pottery is cracked, and outputs 2.50 if true. If this is not true, it then checks the weight, and if the weight is more than or equal to 6, sets the price output to be 40. The final else statement categorizes the left over pieces that are not cracked and not >= 6 weight. Finally, the function will push the pieces into the "soldPottery" array. 
5. In the **Kiln** module, you have a `firePottery()` function. You need to demonstrate how to use the debugger to verify the values of the parameters for that function when your code runs. Use [Loom](https://www.loom.com/) to record your browser window with the developer tools open and show those values.
   >[ Paste your video's public URL here](https://www.loom.com/share/687f8176081742eeb80d58e960f4ef5f)
